...
. Navigate to DynamoDB and check the table called: "[stack_name]-status-table".
. Here, will be an entry for each AMC Lambda function which has either run and finished or run and failed.
. For any failed AMC Lambda functions, navigate to CloudWatch and go to "Dashboards". There is a CloudWatch Dashboard for each AMC Lambda function which will log any "error" level messages.
. For more detailed logs, open the AWS Lambda console, and navigate to the function which failed (replacing the information in brackets):
    `<name_of_stack>-<name_of_failed_function>-<hash>`
